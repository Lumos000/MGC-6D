...
def np_transform_pcd(pcd: np.ndarray, r: np.ndarray, t: np.ndarray) -> np.ndarray:
    # NOTE:
    # float16 is numerically fragile here (squared distances can easily overflow),
    # which may produce inf/NaN in ADD/ADD-S computation for hard frames.
    pcd = np.asarray(pcd, dtype=np.float64)
    r = np.asarray(r, dtype=np.float64)
    t = np.asarray(t, dtype=np.float64)
    if (not np.isfinite(pcd).all()) or (not np.isfinite(r).all()) or (not np.isfinite(t).all()):
        return np.full((pcd.shape[0], 3), np.nan, dtype=np.float64)
    rot_pcd = np.dot(pcd, r.T) + t
    if not np.isfinite(rot_pcd).all():
        return np.full_like(rot_pcd, np.nan, dtype=np.float64)
    return rot_pcd
...
def np_transform_pcd(pcd: np.ndarray, r: np.ndarray, t: np.ndarray) -> np.ndarray:
    # Keep stable precision for metric computation.
    pcd = np.asarray(pcd, dtype=np.float64)
    r = np.asarray(r, dtype=np.float64)
    t = np.asarray(t, dtype=np.float64)
    if (not np.isfinite(pcd).all()) or (not np.isfinite(r).all()) or (not np.isfinite(t).all()):
...
def compute_add(pcd: np.ndarray, pred_pose: np.ndarray, gt_pose: np.ndarray) -> np.ndarray:
    ...
    if (not np.isfinite(model_pred).all()) or (not np.isfinite(model_gt).all()):
        return np.inf
