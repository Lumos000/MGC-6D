...
class Ho3dReader:
  ...
  def get_occ_mask(self,i):
    ...
    index = int(os.path.basename(self.color_files[i]).split('.')[0])
    candidates = [
      # Preferred HO3D evaluation hand-mask layout.
      f'{self.ho3d_root}/evaluation/{video_name}/hand_mask/{index:04d}.png',
      f'{self.ho3d_root}/evaluation/{video_name}/hand_mask/{index:05d}.png',
      # Legacy/generated XMem hand-mask layout.
      f'{self.ho3d_root}/masks_XMem/{video_name}_hand/{index:04d}.png',
      f'{self.ho3d_root}/masks_XMem/{video_name}_hand/{index:05d}.png',
    ]
  ...
  def get_gt_mesh(self, model_dir=None):
    if model_dir is None:
      model_dir = os.environ.get('YCB_MODEL_PATH')
    if model_dir is None:
      raise ValueError('model_dir is required. Pass --ycb_model_path or set YCB_MODEL_PATH.')
...
class YcbineoatReader:
  ...
  def get_mask(self,i):
    ...
    stem = os.path.basename(color_file)
    scene_dir = os.path.dirname(os.path.dirname(color_file))
    candidate_mask_files = [
      color_file.replace('rgb','gt_mask'),
      color_file.replace('rgb','masks'),
      color_file.replace('rgb','mask'),
      f'{scene_dir}/gt_mask/{stem}',
      f'{scene_dir}/masks/{stem}',
      f'{scene_dir}/mask/{stem}',
    ]

    mask = None
    for mask_file in candidate_mask_files:
      if os.path.exists(mask_file):
        mask = cv2.imread(mask_file, -1)
        if mask is not None:
          break
    ...
    if mask is None:
      # Some scenes may not provide per-frame masks.
      return np.zeros((self.H, self.W), dtype=np.uint8)

    if len(mask.shape)==3:
      for c in range(min(3, mask.shape[2])):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
          break
      if len(mask.shape)==3:
