...
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, xyz_map, normal_map=None, mesh_diameter=None, cfg=None, glctx=None, mesh_tensors=None, dataset:PoseRefinePairH5Dataset=None):
  ...
  if torch.is_tensor(ob_in_cams):
    work_device = ob_in_cams.device
  elif isinstance(mesh_tensors, dict) and ('pos' in mesh_tensors) and torch.is_tensor(mesh_tensors['pos']):
    work_device = mesh_tensors['pos'].device
  else:
    work_device = torch.device('cuda')
  B = len(ob_in_cams)
  poseA = torch.as_tensor(ob_in_cams, dtype=torch.float, device=work_device)
  ...
  Ks = torch.as_tensor(K, device=work_device, dtype=torch.float).reshape(1,3,3)
  if cfg['use_normal']:
  ...
  xyz_mapBs = kornia.geometry.transform.warp_perspective(torch.as_tensor(xyz_map, device=work_device, dtype=torch.float).permute(2,0,1)[None].expand(B,-1,-1,-1), tf_to_crops, dsize=render_size, mode='nearest', align_corners=False)  #(B,3,H,W)
  ...
  if cfg['use_normal']:
    ...
    normalBs = kornia.geometry.transform.warp_perspective(torch.as_tensor(normal_map, dtype=torch.float, device=work_device).permute(2,0,1)[None].expand(B,-1,-1,-1), tf_to_crops, dsize=render_size, mode='nearest', align_corners=False)
  else:
    normalAs = None
    normalBs = None
...
class PoseRefinePredictor:
  ...
  def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
    ...
    B_in_cams = torch.as_tensor(ob_centered_in_cams, device=work_device, dtype=torch.float)
    ...
    rgb_tensor = torch.as_tensor(rgb, device=work_device, dtype=torch.float)
    depth_tensor = torch.as_tensor(depth, device=work_device, dtype=torch.float)
    xyz_map_tensor = torch.as_tensor(xyz_map, device=work_device, dtype=torch.float)
    trans_normalizer = self.cfg['trans_normalizer']
    if not isinstance(trans_normalizer, float):
      trans_normalizer = torch.as_tensor(list(trans_normalizer), device=work_device, dtype=torch.float).reshape(1,3)
    ...
    for _ in range(iteration):
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        if self.cfg['trans_rep']=='tracknet':
          ...
          uv_pred = transform_pts(uv_pred_crop, pose_data.tf_to_crops[b:b+bs].inverse().to(work_device))
          center_pred = torch.cat([uv_pred, torch.ones((len(rot_delta),1), dtype=torch.float, device=work_device)], dim=-1)
          center_pred = (pose_data.Ks[b:b+bs].inverse().to(work_device)@center_pred.reshape(len(rot_delta),3,1)).reshape(len(rot_delta),3) * z_pred.reshape(len(rot_delta),1)
    ...
    B_in_cams_out = B_in_cams@torch.tensor(tf_to_center[None], device=work_device, dtype=torch.float)
    ...
    if get_vis:
      ...
      canvas = []
      padding = 2
      pose_data = make_crop_data_batch(self.cfg.input_resize, torch.as_tensor(ob_centered_in_cams, device=work_device), mesh_centered, rgb, depth, K, crop_ratio=crop_ratio, normal_map=normal_map, xyz_map=xyz_map_tensor, cfg=self.cfg, glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, mesh_diameter=mesh_diameter)
